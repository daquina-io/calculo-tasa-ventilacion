...
renderValueBox({
  ...
  ifelse( x < 3 , "red",
  ifelse( x >= 3 & x < 4 , "orange",
  ifelse( x >= 4 & x < 5, "orange",
  ifelse( x >= 5 & x < 6, "greenyellow",
  ifelse( x >= 6 , "green")))))
  }

  valueBox(ach, color = set_color_co2(ach), icon="fa-users")
})
...
output$plot1 <-renderPlot({
  ggplot(NULL, aes(x=tiempo,y=`CO2 [ppm]`)) +
  geom_point(data = df()[input$sample,], size = 5, col = "red") +
  geom_line(data= df(), col = "blue")
})
```
...
renderPlot({
  ggplot(NULL, aes(x=tiempo,y=`CO2 [ppm]`))  + geom_line(data= df(), col = "blue")+
